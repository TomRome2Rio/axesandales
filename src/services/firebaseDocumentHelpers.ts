import type { TerrainBox } from '../types';
...
export const mergeTerrainBoxesWithDefaults = (
  existingBoxes: TerrainBox[],
  defaultBoxes: TerrainBox[]
): TerrainBox[] => {
  const existingIds = new Set(existingBoxes.map(box => box.id));
  return sortFirestoreDocumentsById([
    ...existingBoxes,
    ...defaultBoxes.filter(defaultBox => !existingIds.has(defaultBox.id)),
  ]);
};
